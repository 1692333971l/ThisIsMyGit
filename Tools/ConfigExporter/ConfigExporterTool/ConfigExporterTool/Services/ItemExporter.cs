...
namespace ConfigExporter.Services
{
    ...
    {
        /// <summary>
        /// 读取 Item.xlsx 并导出到两个目标路径
        /// </summary>
        public void Export(string excelPath, string clientOutputPath, string serverOutputPath)
        {
            List<ItemConfig> itemConfigs = ReadItemConfigs(excelPath);

            string json = JsonSerializer.Serialize(itemConfigs, new JsonSerializerOptions
            {
                WriteIndented = true
            });
        ...
        /// 从 Excel 中读取道具配置列表
        /// </summary>
        private List<ItemConfig> ReadItemConfigs(string excelPath)
        {
            ...
            if (rows.Count < 3)
            {
                throw new Exception("Item.xlsx 至少需要 3 行：说明行、字段行、数据行。");
            }
            ...
            for (int i = 2; i < rows.Count; i++)
            {
                ...
                {
                    ItemId = GetIntCellValue(row, fieldIndexMap, "ItemId", sharedStringPart),
                    ItemName = GetStringCellValue(row, fieldIndexMap, "ItemName", sharedStringPart),
                    ItemType = GetIntCellValue(row, fieldIndexMap, "ItemType", sharedStringPart),
                    MaxStackCount = GetIntCellValue(row, fieldIndexMap, "MaxStackCount", sharedStringPart),
                    SellPrice = GetIntCellValue(row, fieldIndexMap, "SellPrice", sharedStringPart),
                    Quality = GetIntCellValue(row, fieldIndexMap, "Quality", sharedStringPart),
                    IconPath = GetStringCellValue(row, fieldIndexMap, "IconPath", sharedStringPart),
                    Description = GetStringCellValue(row, fieldIndexMap, "Description", sharedStringPart),
                    CanUse = GetIntCellValue(row, fieldIndexMap, "CanUse", sharedStringPart),
                    UseEffectType = GetIntCellValue(row, fieldIndexMap, "UseEffectType", sharedStringPart),
                    UseEffectValue = GetIntCellValue(row, fieldIndexMap, "UseEffectValue", sharedStringPart)
                };
